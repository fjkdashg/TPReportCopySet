...
{
    public partial class BRepWork : Form
    {
        WorkSet WS;
        DataTable ChildDT;
        ...
        public BRepWork(WorkSet _ws,DataTable _ChildDT)
        {
            ...
            using (StreamReader sr = new StreamReader(this.BCD_Path.Text))
            {
                string line;
            ...
            using (StreamReader sr = new StreamReader(this.BZCFZ_Path.Text))
            {
                string line;
                while ((line = sr.ReadLine()) != null)
                {
            ...
            using (StreamReader sr = new StreamReader(this.BLR_Path.Text))
            {
                string line;
                while ((line = sr.ReadLine()) != null)
                {
        ...
        public void Work_Do(object sender, DoWorkEventArgs e)
        {
            ...
            if (_trc.state)
            {
                ...
                foreach (DataGridViewRow vr in DGV1.Rows)
                {
                    ...
                    {
                        using (StreamWriter sw = new StreamWriter(this.BCD_Path.Text, append: true))
                        {
                        ...
                        BW.ReportProgress(1);
                        using (StreamWriter sw = new StreamWriter(this.BZCFZ_Path.Text, append: true))
                        {
                        ...
                        BW.ReportProgress(1);
                        using (StreamWriter sw = new StreamWriter(this.BLR_Path.Text, append: true))
                        {
